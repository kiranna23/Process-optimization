...
base_seed <- digest2int('Кириленко Анна')
listN <- function(...){
    anonList <- list(...)
...
task2 <- list(
  f = function(par){
    x <- par[1]
    y <- par[2]
    9 * x ^ 2 + 16 * x * y + 9 * y ^ 2 + 6 * exp(-((((x-2)^2)/1) + ((y+3)^2)/4)) + 10 * exp(-((((x)^2)/3) + ((y-1)^2)/1)) + 6 * exp(-((((x-1)^2)/4) + ((y+2)^2)/2))
  },
  
 gradF = function(par)
    {
   xy <- list(x = par[1], y = par[2])
    f1 = expression(9 * x ^ 2 + 16 * x * y + 9 * y ^ 2 + 6 * exp(-((((x-2)^2)/1) + ((y+3)^2)/4)) + 10 * exp(-((((x)^2)/3) + ((y-1)^2)/1)) + 6 * exp(-((((x-1)^2)/4) + ((y+2)^2)/2)))
    dx <- D(f1, 'x')
    dy <- D(f1, 'y')
    c(eval(dx, envir = xy)[1], eval(dy, envir = xy)[1])
  },
  ...
  hess_f = function(par){
      ...
      dydy <- D(dy, 'y')
      dxdy <- D(dx, 'y')
      rbind(c(eval(dxdx, envir = xy)[1], eval(dxdy, envir = xy)[1]),
             c(eval(dxdy, envir = xy)[1], eval(dydy, envir = xy)[1]))
  }
...
row <- data.frame(
  ...
  method = "mma",
  num_x = c(2),
  iter = c(Rrs$iterations),
  Func = c(Rrs$objective)
)
...
task5 <- list(
  f = function(par){
    ...
    (12*x + 14*y - 12)^4 + (2*x + 2*y -5)^4
  }, 
  
  gradF = function(par){
    xy <- list(x = par[1], y = par[2])
    f1 = expression((12*x + 14*y - 12)^4 + (2*x + 2*y -5)^4)
    dx <- D(f1, 'x')
    dy <- D(f1, 'y')
    c(eval(dx, envir = xy)[1], eval(dy, envir = xy)[1])
  }
...
row <- data.frame(
  task = c(7),
  method = "Nelder-Mead (DeckkersAarts)",
  num_x = c(2),
  iter = c(Res$iterations),
  Func = c(Res$objective)
)
